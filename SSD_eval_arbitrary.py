...
# --- Constants ---
# Define the model version you are working with
MODEL_VERSION = "eowremm8" # Or "mfcoy29e", etc.
...
def run_evaluation(
    ...
    latent_dim: int = 12,
    encoders_hidden_dim: int = 128, # Default encoder hidden dim
    decoders_hidden_dim: int = 256, # Default decoder hidden dim
    debug_mode: bool = False,
    encoders_n_layers: int = 0, # Optional, adjust if needed
    decoders_n_layers: int = 0 # Optional, adjust if needed
    ...
    my_hparams = {"temperature": 0.7, "alpha": 1.3} # Example hparams, adjust if needed
    ...
    config = {
        "domains": [
            LoadedDomainConfig(
                ...
                checkpoint_path=Path("checkpoints/domain_v.ckpt"), # Ensure path is correct
            ),
            LoadedDomainConfig(
                domain_type=attr_domain_type,
                checkpoint_path=Path("checkpoints/domain_attr.ckpt"), # Ensure path is correct
                args=my_hparams,
            ),
        ],
        "global_workspace": {
...
def visualize_multiple_distributions_overlay(
    ...
    alpha: float = 0.6, histtype: str = 'stepfilled', density: bool = True ):
...
# SEED_0 = "eowremm8" # Example seed for the first model
# SEED_1 = "mz2uvxqs"
# if __name__ == "__main__":

#     # --- Configuration for Multiple Model Runs ---
#     # !! IMPORTANT: Update gw_checkpoint_path for EACH entry !!
#     model_configs = [
#         { "run_id": "seed0", "full_attr": False, "gw_checkpoint_path": f"/home/alexis/Desktop/checkpoints/gw/version_colorTrue_{SEED_0}/last.ckpt" }, # Replace!
#         { "run_id": "seed1", "full_attr": False, "gw_checkpoint_path": f"/home/alexis/Desktop/checkpoints/gw/version_colorTrue_{SEED_1}/last.ckpt" },
#         { "run_id": "seed2", "full_attr": False, "gw_checkpoint_path": "/home/alexis/Desktop/checkpoints/gw/version_colorFalse_eowremm8/last.ckpt" }, # Replace!
#         # Add more...
#     ]

#     if len(model_configs) < 2: LOGGER.error("Need >= 2 model configs."); exit()

#     evaluation_parent_dir = f"./evaluation_runs_testing"
...
#             output_parent_dir=evaluation_parent_dir, debug_mode=DEBUG_MODE
#         )
#         if eval_result:
...
#     if analysis_results_first_model is None: LOGGER.error("Eval failed for first model. Exiting."); exit()
#     if len(successful_runs) < 2: LOGGER.error("Need >= 2 successful evals. Exiting."); exit()
#     LOGGER.info(f"\nSuccessfully evaluated models: {successful_runs}")
...
#     # ... (Logging summary of significant attributes - same as before) ...
#     if not all_significant_attributes_union:
#          LOGGER.info("No attributes met the filtering criteria (p < {significance_alpha_filter}) in ANY path of the first model.")
#     else:
#         LOGGER.info(f"Attributes showing significance (p < {significance_alpha_filter}) in at least one path (and their bins):")
#         for attr, bins_set in all_significant_attributes_union.items(): LOGGER.info(f"  - {attr}: {sorted(list(bins_set))}")

#     # --- Perform Pairwise Cross-Model Comparison (Conditionally Saving Details) ---
#     if not all_significant_attributes_union:
#         LOGGER.info("Skipping pairwise cross-model comparison.")
#     else:
#         LOGGER.info(f"\n--- Starting Pairwise Cross-Model Comparison for {len(successful_runs)} Models ---")
#         LOGGER.info(f"--- Details will be saved ONLY if comparison KS p-value < {significance_alpha_compare} ---")
#         comparison_output_dir = f"./cross_model_comparison_{MODEL_VERSION}_N{len(successful_runs)}_filtered_p{significance_alpha_filter}_sig_save_p{significance_alpha_compare}"
...
#         else:
#             LOGGER.info("No significant differences found between model pairs for the filtered attributes/bins/paths.")
